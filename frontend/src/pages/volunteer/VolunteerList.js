    ...
    <div className={style.myContainer}>
      ...
      <div className={style.searchBar}>
        ...
        <div className={style.bottom}>
          ...
          <button className={style.searchBtn} onClick={getList}>
            조회
          </button>
        </div>
      </div>
      ...
      {isLogin ? (
        <p className={style.writeBtn} onClick={goToWrite}>
          🖊 글쓰기
        </p>
      ) : null}
              ...
              volunteers.map((volunteer) => {
                return (
                  <tr
                    key={volunteer.seq}
                    onClick={() => {
                      goToVolunteer(volunteer.seq);
                    }}
                  >
                    <td>
                      {workStatus(volunteer.status)}&nbsp;
                      {leftDays(volunteer.endDate, volunteer.status)}
                    </td>
                    <td>{volunteer.title}</td>
                    <td>
                      {volunteer.approveCount}/{volunteer.maxParticipantCount}
                    </td>
      ...
      <Paging total={totalItemCount} limit={10} page={page} setPage={setPage} />
    </div>
  );
}
