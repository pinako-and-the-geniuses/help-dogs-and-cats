...
function TeamManage(props) {
  ...
  const getApprove = async (seq) => {
    ...
    partyApprove(true, seq).then(getParticipants());
    getParticipants();
  };

  const cancleApprove = (seq) => {
    partyApprove(false, seq).then(getParticipants());
    getParticipants();
  };
        ...
        participants.map((p) => {
          return (
            <div className={style.person} key={p.seq}>
              <p>{p.nickname}</p>
              {p.seq === memSeq ? null : p.approve ? (
                <button
                  className={`${style.btnCancle} ${style.btn}`}
                  type="button"
                  onClick={() => {
                    cancleApprove(p.seq);
                  }}
                ...
                <button
                  className={`${style.btnAprv} ${style.btn}`}
                  type="button"
                  onClick={() => {
                    getApprove(p.seq);
                  }}
